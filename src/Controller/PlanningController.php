...
class PlanningController extends AbstractController
{
...

    {
        return $this->render('planning/index.html.twig', [
            ...
            'ActifAteliers'    => $planningRepository->findActifAtelier(),
            'counts'       => $planningRepository->countUsagerByAtelier(),

        ]);
    }

    #[Route('/liste_usagers//atelier/{ateliers}', name: 'show_liste_usagers_par_atelier', methods: ['GET'])]
    public function listeUsager(PlanningRepository $planningRepository, Planning $planning, String  $ateliers): Response
    {
    ...
    public function newUsager(Request $request, PlanningRepository $planningRepository, Atelier $atelier): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
           ...
           if($planningRepository->add($planning)){
            $this->addFlash(
                'success',
                "L'inscription de l'usager est effectué avec succès ."
             );
            }else{
                $this->addFlash(
                    'error',
                    "Erreur - Inscription non effectué"
                 );
            }
